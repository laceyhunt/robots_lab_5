...
home_position = [0.0,0.0,0.0,0.0,-90.0,0.0]
bill.write_joint_pose(home_position)
def home_robot():
    bill.write_joint_pose(home_position)
    time.sleep(1)
def pickup_die(x, y, z=122.70266723632812):
    bill.write_cartesian_position([x, y, z + 100.0, -179.0, 0, -130.0])
    time.sleep(0.25)
    bill.write_cartesian_position([x, y, z, -179.0, 0, -130.0])
    time.sleep(0.25)
...
def save_H_mtx(H):
    np.savetxt("homography_mtx_bill.txt", H)

def read_H_mtx():
    H_loaded = np.loadtxt("homography_mtx_bill.txt")
    return H_loaded
...
def find_homography_matrix():
    ...
    #bill.write_cartesian_position([838.9059448242188, -353.26605224609375, 123.34469604492188, -179.0, 0, -130.0])
    offset = 300.0
    px = 550.0
    py = -550.0
    bill.write_cartesian_position([px, py, 223.70266723632812, -179.0, 0, -130.0])
    time.sleep(1)
    bill.write_cartesian_position([px, py, 123.70266723632812, -179.0, 0, -130.0])
    ...
    for i in range(0,4):
        ...
        bill.write_cartesian_position([float(real_points[(i) % 4][0]), float(real_points[(i) % 4][1]), 223.70266723632812, -179.0, 0, -130.0])
        bill.write_cartesian_position([float(real_points[(i + 1) % 4][0]), float(real_points[(i + 1) % 4][1]), 123.70266723632812, -179.0, 0, -130.0])
        time.sleep(1)
        bill.write_cartesian_position([float(real_points[(i+1) % 4][0]), float(real_points[(i+1) % 4][1]), 123.70266723632812, -179.0, 0, -130.0])
        bill.onRobot_gripper(**open_gripper_params)
        time.sleep(1)
        bill.write_cartesian_position([float(real_points[(i + 1) % 4][0]), float(real_points[(i + 1) % 4][1]), 223.70266723632812, -179.0, 0, -130.0])
        home_robot()
